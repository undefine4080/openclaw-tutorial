...
### 6.1 案例 1：发送消息后返回 pairing 配对码，如何配对？
...
- OpenClaw 的安全机制：`dmPolicy: "pairing"`
- 未授权用户的消息不会被处理，只返回配对码
- 防止恶意用户访问你的 AI 助手
...
配对成功后，再次发送消息，机器人会正常回复。

**注意事项**：

- 配对码有效期 1 小时，超时需重新获取
- 每个 channel 最多同时保留 3 个待处理配对请求
- 配对成功后，用户信息会持久化到 `~/.openclaw/credentials/feishu-allowFrom.json`
...
- 确认已申请以下权限：
  ...
  - `im:message.p2p_msg`（私聊）
  - `im:message.group_msg`（群聊）

**4. 检查配置顺序**

**常见错误**：先配置飞书后台，再启动 Gateway
...
解决方法：重新启动 Gateway，并重新保存飞书后台的事件订阅配置。

### 6.3 案例 3：权限错误（缺少必要权限）

**现象**：

- 机器人收到消息，但无法回复
- 日志中显示：`permission denied` 或 `缺少权限`
...
如果 `groupPolicy: "allowlist"`，需要将群组 ID 添加到 `groupAllowFrom`：
...
如果配置了 `groups.<id>.allowFrom`，需要添加用户 ID：
...
⚠️ **飞书平台的限制**：不同的机器人获取到的用户 ID 可能不同！
...
1. 在日志中查看用户的真实 ID：
   ...
   openclaw logs --follow
   ```
2. 将真实的用户 ID 添加到 `allowFrom` 中

### 6.6 案例 6：依赖缺失导致崩溃

**现象**：

- 启动 Gateway 时报错：`Cannot find module '@larksuiteoapi/node-sdk'`
- Gateway 无法启动

**原因**：

- 使用了飞书官方插件（`@larksuiteoapi/node-sdk`），但未安装依赖

**解决方案**：

**方案 1：使用 OpenClaw 内置插件（推荐）**

OpenClaw 内置了飞书支持，无需安装额外依赖。

**方案 2：安装官方插件**

如果需要使用飞书官方插件（支持文档、多维表格、日历、任务）：
...
在完成接入后，让我们深入了解飞书平台的优势，以及为什么它适合作为 AI 助手的承载平台。

### 7.1 企业级稳定性和可用性

飞书是字节跳动的核心协作产品，服务了数百万家企业，包括：
...
- 文档（Docs）：读取、编辑、创建文档
- 多维表格（Bitable）：类似 Airtable，支持数据管理
- 日历（Calendar）：事件管理、日程同步
- 任务（Task）：任务分配、进度跟踪

**这些功能都可以通过 OpenClaw 的飞书插件实现，未来可以扩展更多能力。**
...
- 最小权限原则：应用默认无权限，需显式申请
- 权限审核：商店应用需要审核，企业自建应用自动通过
- 权限范围：可以限制到部门、用户级别
...
- 飞书审批流
- 飞书自动化（类似 Zapier）
- 第三方应用集成（Jira、GitHub、Jenkins 等）

**这意味着你的 AI 助手可以与企业现有系统无缝对接，提供更强大的能力。**

### 7.5 免费额度与成本优势

飞书开放平台对企业自建应用的免费额度非常慷慨：

**免费额度**：

- API 调用次数：几乎无限制（企业自建应用）
- 消息发送次数：几乎无限制
- 文件存储空间：足够个人/小团队使用

**对比其他平台**：

- Slack：免费版限制消息历史（90 天）
- Discord：免费，但国内访问受限
- Telegram：免费，但国内访问不稳定

**对于个人使用、小团队协作，飞书的成本优势明显。**
...
恭喜你完成了 OpenClaw 接入飞书的完整流程！让我们回顾一下核心关键点：
...
- 使用 WebSocket 长连接模式，无需公网 IP、无需内网穿透
- 配置顺序很重要：先启动 Gateway，再配置飞书后台
- 理解 `dmPolicy` 和 `groupPolicy` 的区别
- 权限申请后必须发布版本

**3. 安全机制**

- `dmPolicy: "pairing"` 是最安全的私聊访问策略
- 配对码有效期 1 小时，超时需重新获取
- App Secret 必须妥善保管
...
**1. 卡片消息（Interactive UI）**

飞书支持富交互的卡片消息，可以创建更丰富的 UI：
...
通过 OpenClaw 的 workspace 机制，可以实现多租户：

- 不同的用户使用不同的 workspace
- 隔离数据和权限
- 适合团队协作
...
- [OpenClaw 飞书配置文档](https://docs.openclaw.ai/zh-CN/channels/feishu)
...
- [OpenClaw 实战指南：多平台部署](https://apifox.com/apiskills/openclaw-docker-compose-feishu-dingtalk-wecom/)
- [飞书 × OpenClaw 接入指南](https://adg.csdn.net/69a286f10a2f6a37c594241a.html)
- [GitHub - 飞书配置保姆级指南](https://github.com/AlexAnys/openclaw-feishu)
- [腾讯云 - OpenClaw 接入飞书教程](https://cloud.tencent.com/developer/article/2638276)

**中文社区**：

- [OpenClaw 安全策略：DM 配对与权限控制](https://www.cnbugs.com/post-7133.html)
- [OpenClaw 安全机制深拆](https://www.freebuf.com/articles/473605.html)
- [OpenClaw 接入通讯 App 打造 AI 助手](https://zhuanlan.zhihu.com/p/2015868004152393910)

---

## 结语

从零到一接入飞书，看似复杂，其实只需要搞清楚几个核心关键点：

1. **WebSocket 长连接**：无需公网 IP、无需内网穿透
2. **配置顺序**：先启动 Gateway，再配置飞书后台
3. **安全机制**：`dmPolicy: "pairing"` 是最安全的策略
4. **权限管理**：申请权限后必须发布版本

独立部署虽然需要动手配置，但收获远超想象：

- 数据完全掌握在自己手中
- 学到了 WebSocket、权限体系、安全机制等技术
- 理解了如何设计一个可扩展的 AI 助手平台

**先把手弄脏，才能真正理解。** 希望这篇教程能帮助你顺利接入飞书，开启你的个人 AI 助手之旅。

如果你在接入过程中遇到问题，欢迎查阅本文的踩坑案例章节，或者在 OpenClaw 社区提问。
